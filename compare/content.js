...
function toggleTranslationMode() {
    if (isActive) {
        ...
        // Check if SaaS is enabled or API key is set before activating
        try {
            if (!isChromeAPIAvailable()) throw new Error('Chrome API not available');

            chrome.storage.sync.get(['useSaasApi', 'saasToken', 'apiKey'], function (result) {
                try {
                    console.log('Checking authentication status:', {
                        useSaasApi: result.useSaasApi,
                        hasSaasToken: !!result.saasToken,
                        hasApiKey: !!result.apiKey
                    });
                    
                    // If SaaS is enabled and user is logged in, activate immediately
                    if (result.useSaasApi && result.saasToken) {
                        console.log('Using SaaS API, activating translation mode');
                        activateTranslationMode();
                        chrome.storage.sync.set({ isActive: true });
                        return;
                    }
                    
                    // If SaaS is enabled but not logged in, guide user to login
                    if (result.useSaasApi && !result.saasToken) {
                        alert('请先登录Pointer云服务。即将打开设置页面进行登录。');
                        // Open options page
                    ...
                    // If not using SaaS, check for API key
                    if (!result.apiKey) {
                        alert('Please set your API key in the extension settings or enable Pointer cloud service');
                        // Open options page
                        if (chrome.runtime && chrome.runtime.openOptionsPage) {
                            chrome.runtime.openOptionsPage();
                        }
                    ...
                    console.log('Using API key, activating translation mode');
                    activateTranslationMode();
                    chrome.storage.sync.set({ isActive: true });
                } catch (error) {
                    console.error('Error activating translation mode:', error);
...
function translateSimpleSelection(range, selectedText) {
    ...
    try {
        ...
        chrome.storage.sync.get(['useSaasApi', 'saasToken', 'apiKey'], function (result) {
            console.log('Translation request - checking auth:', {
                useSaasApi: result.useSaasApi,
                hasSaasToken: !!result.saasToken,
                hasApiKey: !!result.apiKey
            });
            
            // If SaaS is enabled and user is logged in, proceed with translation
            if (result.useSaasApi && result.saasToken) {
                console.log('Using SaaS for translation');
                // Send translation request directly
            ...
            // If SaaS is enabled but not logged in, guide user to login
            if (result.useSaasApi && !result.saasToken) {
                console.log('SaaS enabled but not logged in');
            ...
            console.log('Using API key for translation');
        ...
        function sendTranslationRequest() {
            ...
            chrome.runtime.sendMessage({
                ...
                try {
                    ...
                    // Show cache hit feedback if applicable
                    if (response.fromCache) {
                        console.log('Translation retrieved from cache');
                        // Optionally show a small cache indicator
                        const cacheIndicator = document.createElement('div');
                        cacheIndicator.textContent = '⚡';
                        cacheIndicator.style.position = 'absolute';
                        cacheIndicator.style.left = `${window.scrollX + range.getBoundingClientRect().left - 20}px`;
                        cacheIndicator.style.top = `${window.scrollY + range.getBoundingClientRect().top}px`;
                        cacheIndicator.style.fontSize = '12px';
                        cacheIndicator.style.zIndex = '10002';
                        cacheIndicator.style.color = '#00ff00';
...
function showLoadingNearRange(range) {
    ...
    indicator.innerHTML = `
        <div style="display: flex; align-items: center; gap: 5px;">
            <div style="width: 12px; height: 12px; border: 2px solid #ccc; border-top: 2px solid #3498db; border-radius: 50%; animation: spin 1s linear infinite;"></div>
            <span>翻译中...</span>
        </div>
    `;
    indicator.style.position = 'absolute';
    indicator.style.left = `${window.scrollX + rect.left}px`;
    indicator.style.top = `${window.scrollY + rect.top - 30}px`; // Position above
